...
- **특징**: Windows 가상 데스크톱 전환 시 오버레이가 따라가지 않도록 `skipTaskbar`를 끄고, 표시 시에는 다시 `fullscreen`을 사용합니다.
- **특징**: 오버레이는 일반 top-level 창으로 유지하기 위해 작업 표시줄 숨김 전용 스타일(`skipTaskbar`)을 사용하지 않습니다.
