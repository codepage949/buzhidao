...
에러 메시지, UI 문구, 테스트 데이터에 로컬 개발 환경 절대 경로를 포함하면
사용자 노출과 문서/테스트 오염이 동시에 발생한다.

→ 파일 경로가 필요한 내부 진단 로그가 아니라면
오류 원문 자체에서 경로를 제거할 것.
UI 레이어에서만 가리는 것으로 끝내지 말고,
테스트 fixture와 변경 문서에도 실제 로컬 경로를 남기지 말 것.
커밋 전에는 변경 파일에 개인 홈 디렉터리 기반 절대 경로가 남아 있지 않은지 확인하고,
예시는 `<project-root>` 같은 일반화된 경로로 작성할 것.
...
### Linux OpenCV SDK 구성은 setup 스크립트 책임으로 둘 것

릴리즈 smoke가 통과했는데 실제 Linux 실행 파일이 `libopencv_core.so.*`를 찾지 못하면
패키징 단계만 보지 말고 `setup_paddle_inference.py`가 Linux OpenCV SDK를 canonical 경로에
직접 구성하는지 먼저 확인해야 한다.

workflow에서 임시 OpenCV SDK를 수동으로 만들면 링크용 `.so`와 런타임 SONAME `.so.*`
복사 규칙이 흩어져 smoke와 실제 실행의 조건이 달라질 수 있다.

→ Windows/Linux 모두 OpenCV SDK 구성 책임은 `setup_paddle_inference.py`에 모으고,
릴리즈 workflow는 준비 스크립트를 호출만 하게 유지할 것.
Linux에서는 `libopencv-dev` 설치 결과에서 헤더와 `libopencv_*.so*`를 함께 canonical SDK로 가져와야 한다.

### CI Python 스크립트는 표준 입출력을 UTF-8로 재설정할 것

Windows GitHub Actions runner의 Python stdout/stderr 기본 인코딩이 cp1252일 수 있다.
스크립트가 한글 상태 메시지를 출력하면 `UnicodeEncodeError`로 실제 검증 결과와 무관하게 실패한다.

→ workflow에서 직접 실행되는 Python 스크립트는 `main()` 시작 시
