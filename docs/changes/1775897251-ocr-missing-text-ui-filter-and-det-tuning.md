...
전체 화면을 축소해서 한 번에 det에 넣는 대신,
큰 이미지는 `OCR_DET_RESIZE_LONG` 크기 타일로 나눠 원본 해상도에 가깝게 처리한다.

- 목적: 작은 글자나 긴 화면에서 축소 손실 완화
- 중복 박스는 AABB IoU 기준으로 제거

## 3. UI source filter 제거
...
- `src/ocr/det.rs`: threshold 파라미터 전달, 타일 분할 및 중복 제거 추가
...
- `cargo test tile_offsets_1920_너비는_두_타일 -- --nocapture`
