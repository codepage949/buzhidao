...
import { nextCloseSuppressed } from "./close";
import { selectionOutcome, type SelectionRect } from "./selection";
import { useListenerCleanup, useWindowKeydown } from "../../lib/app-hooks";
import { measureOverlayOcrElapsedMs } from "./timing.ts";

type OcrResultPayload = {
  groups: DetectionTraceGroup[];
  debug_detections?: [BoundingBox, string, number, boolean][];
  orig_width: number;
  orig_height: number;
  debug_trace?: boolean;
};
...
type State =
  ...
  | { kind: "selecting" }
  | { kind: "ready"; ocr: OcrResultPayload; elapsedMs: number | null }
...
function OverlayApp() {
  ...
  useListenerCleanup(() => [
      ...
      listen<OcrResultPayload>("ocr_result", (e) => {
        const elapsedMs = measureOverlayOcrElapsedMs(
          loadingStartedAtRef.current,
          performance.now(),
        );
        loadingStartedAtRef.current = null;
        setState({ kind: "ready", ocr: e.payload, elapsedMs });
        setSelectionStart(null);
        setSelectionRect(null);
        selectionResumeStateRef.current = null;
        suppressNextCloseRef.current = nextCloseSuppressed(
  ...
  const groups = useMemo(
    () => (state.kind === "ready" ? state.ocr.groups : []),
    [state],
  );
  const debugTraceEnabled =
    state.kind === "ready" && (state.ocr.debug_trace ?? false);
  const rawItems = useMemo(
    () =>
      debugTraceEnabled
        ? (state.ocr.debug_detections ?? []).map(([bounds, text, score, accepted]) => {
            return {
        ...
        : [],
    [debugTraceEnabled, state],
  );

  useEffect(() => {
    if (!import.meta.env.DEV || state.kind !== "ready" || !debugTraceEnabled) return;
    console.log("[OCR][overlay] raw items", rawItems);
    console.log("[OCR][overlay] grouped items", groups);
  }, [debugTraceEnabled, groups, rawItems, state]);
  ...
  const elapsedLabel =
    state.kind === "ready" && state.elapsedMs !== null
      ? `${state.elapsedMs} ms`
      : null;
