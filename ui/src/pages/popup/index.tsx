...
type TranslationStartPayload = { requestId: number };
type TranslationContentPayload = { requestId: number; content: string };
...
function PopupApp() {
  ...
  useListenerCleanup(() => [
      listen<TranslationStartPayload>("translating", (e) => {
        activeRequestIdRef.current = e.payload.requestId;
        setStatus("translating");
        setContent("");
      }),
      listen<TranslationContentPayload>("translation_result", (e) => {
        if (!shouldApplyPopupTranslationEvent(activeRequestIdRef.current, e.payload.requestId)) {
          return;
        }
        setStatus("done");
        setContent(e.payload.content);
      }),
      listen<TranslationContentPayload>("translation_error", (e) => {
        if (!shouldApplyPopupTranslationEvent(activeRequestIdRef.current, e.payload.requestId)) {
          return;
        }
        setStatus("error");
