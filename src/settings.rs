...
pub(crate) fn validate_capture_shortcut(value: &str) -> Result<(), String> {
    Shortcut::from_str(value).map(|_| ()).map_err(|_err| {
        format!("캡처 단축키 형식이 올바르지 않습니다. 예: Ctrl+Alt+A, Cmd+Shift+A")
    })
}
...
fn merge_env_example_with_existing(env_example: &str, existing: &str) -> String {
    let newline = detect_newline(env_example)
        .or_else(|| detect_newline(existing))
        .unwrap_or("\n");
...
mod tests {
    ...
    fn capture_shortcut_검증은_잘못된_accelerator를_거부한다() {
        let err =
            validate_capture_shortcut("Ctrl++A").expect_err("잘못된 단축키는 거부되어야 한다");
